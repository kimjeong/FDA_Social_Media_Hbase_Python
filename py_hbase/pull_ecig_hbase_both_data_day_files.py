...
# Go through every row passed back
    ...
    day_id = 9
    flu_list = ['flu','influenza','vaccine','quadrivalent','flushot','flumist','shot','mist']
    print str(day_id)
         ...
         for cell, cell_t in zip(row, row_t):
             ...
             if columnname_t == cfname_text + ':' + 'text':
                 ...
                 for tw_sp in tw_text_t_sp:
                     if tw_sp.lower().replace('\"','').replace(',','').replace('.','').replace('#','').replace('?','').replace(':','').replace(';','').replace('/','').replace('}','').replace('!','').replace('-','') in flu_list:
                         flu_flag = True
                         break
                 if flu_flag:
                      continue
